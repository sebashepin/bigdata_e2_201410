...
function renderGraphData(canvasId,data){
	var sys = arbor.ParticleSystem(1, 1, 1, 1);
	sys.renderer = createGraphRenderer(canvasId);
	$.each(data.nodes, function(name, info){
			info.label=name;
		})
	sys.merge({'nodes':data.nodes, 'edges':data.edges});
	sys.parameters({stiffness:1000});
}

function createGraphRenderer(canvasId){
		...
		redraw:function(){
			...
			particleSystem.eachEdge(function(edge, pt1, pt2){
				...
				if (color!==undefined || weight!==undefined){
					...
					ctx.strokeStyle = palette[edge.data.level];
