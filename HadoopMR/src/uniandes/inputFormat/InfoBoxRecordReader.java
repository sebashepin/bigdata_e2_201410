 ...
 */
public class InfoBoxRecordReader extends RecordReader<LongWritable, Text> {
	...
	private static final Log LOG = LogFactory.getLog(InfoBoxRecordReader.class);
	...
	public void initialize(InputSplit genericSplit, TaskAttemptContext context)
		...
		this.delimiterRegexStart = "^{{[Ii]nfobox.*";
		this.delimiterRegexEnd = "}}$";
 
		delimiterPatternStart = Pattern.compile(delimiterRegexStart);
		delimiterPatternEnd = Pattern.compile(delimiterRegexEnd);
	...
	private int readNext(Text text, int maxLineLength, int maxBytesToConsume)
		...
		for (int i = 0; i < maxBytesToConsume; i++) {
			...
			offset += offsetTmp;
			Matcher mStart = delimiterPatternStart.matcher(tmp.toString());
			Matcher mEnd = delimiterPatternEnd.matcher(tmp.toString());
